...
def parse_report_alternative(content, output_html_path, txt_file_path):
    ...
    try:
        ...
        print(f"❌ Alternative parsing also failed for {txt_file_path}: {str(e)}")
        return False
...
```
your_directory/
├── report_processor.py     (the updated Python script)
├── run_batch.bat          (simple batch runner)
├── reports/               (put your .txt files here)
│   ├── report1.txt
│   ├── report2.txt
│   └── ...
...
Perfect! Now I can see exactly what needs to be modified. Here are the **specific changes needed** to switch from `.txt` to `.html` reports in your existing codebase:

## 1. Update Flask Backend (app.py)

**Replace lines 458-475** (current TXT report endpoints) with HTML versions:
...
def list_reports():
    try:
        # List both .txt and .html for transition period
        txt_files = [f for f in os.listdir(app.config['REPORTS_DIR']) if f.endswith('.txt')]
        html_files = [f for f in os.listdir(app.config['REPORTS_DIR']) if f.endswith('.html')]
        # Prefer HTML files, fallback to TXT
        files = html_files if html_files else txt_files
...
def get_report(filename):
    # Allow both .txt and .html during transition
    if not filename.endswith(('.txt', '.html')):
        return jsonify({"error": "Only .txt or .html reports are allowed"}), 400
    
    try:
        if filename.endswith('.html'):
            return send_from_directory(app.config['REPORTS_DIR'], filename, mimetype='text/html')
        else:
...
## 2. Update Frontend Report Loading (main.js)

**Replace the `loadReportData` function** (lines 390-430) with HTML version:

```javascript
// REPORT LOADING WITH ASSET SERVER FALLBACK - HTML VERSION
async function loadReportData(imageId) {
    try {
        showReportLoading(true);
        
        // Try HTML report first from asset server
        const htmlReportUrl = `${getAssetUrl(`reports/${imageId}.html`)}`;
        const res = await fetch(htmlReportUrl, {
        ...
        // HTML not found, try TXT as fallback (temporary during transition)
        console.log("HTML report not found, trying TXT fallback...");
        const txtReportUrl = `${getAssetUrl(`reports/${imageId}.txt`)}`;
        const txtRes = await fetch(txtReportUrl, {
            signal: abortController.signal,
        });

        if (txtRes.ok) {
            const reportText = await txtRes.text();
            if (reportContent) {
        ...
        // Both failed, try backend API as last resort
        throw new Error("Report not found on asset server");
        ...
        if (error.name !== "AbortError") {
            ...
            try {
                ...
                const localRes = await fetch(`/api/reports/${imageId}.html`, {
                    signal: abortController.signal,
                });
                if (localRes.ok) {
                    const localReportHtml = await localRes.text();
                    if (reportContent) {
                        reportContent.innerHTML = localReportHtml;
                    }
                    return;
                }
                
                // Final fallback to TXT via backend
                const localTxtRes = await fetch(`/api/reports/${imageId}.txt`, {
                    signal: abortController.signal,
                });
                if (localTxtRes.ok) {
                    const localReportText = await localTxtRes.text();
                    if (reportContent) {
                    ...
                    if (reportContent) {
                        reportContent.innerHTML = `
                            <div class="error-message">
                                ...
                                <p>No analysis report available for image: ${imageId}</p>
                                <p class="error-detail">All report sources failed to load</p>
                            </div>
                        `;
                    }
                }
...
## 3. Update Report Content CSS (Add to your styles)

Add this to your CSS to ensure HTML reports display properly:
...
.report-content .report-container {
  font-family: 'Courier New', monospace;
  background: white;
  border-radius: 5px;
  padding: 15px;
...
.report-content .modern-table {
  width: 100%;
  border-collapse: collapse;
  margin: 10px 0;
}

.report-content .modern-table th,
.report-content .modern-table td {
...
.report-content .info-row {
  display: table-row;
  margin: 2px 0;
}

.report-content .info-key {
  display: table-cell;
  font-weight: bold;
  padding-right: 10px;
  min-width: 200px;
...
1. **First, run batch conversion** (if not done):
   ```bash
   python report_processor.py batch
   ```

2. **Apply the code changes above** to your 3 files

3. **Restart your Flask application**

4. **Test the integration** by selecting an image in the dashboard

## Key Changes Summary:

- **Backend**: Now serves both `.html` and `.txt` reports during transition
- **Frontend**: Tries HTML first, falls back to TXT with markdown parsing
- **Asset Server**: Continues serving from `D:\nginx-traefik-static\assets\reports\`
- **Progressive Migration**: You can convert reports gradually without breaking existing functionality

The system will automatically prefer HTML reports when available and gracefully fall back to TXT for any unconverted reports. Once all reports are converted, you can remove the TXT fallback logic.
